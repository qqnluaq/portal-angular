...
@Component({
    selector: "wfrm-base-error-panel",
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (changes.errorState) {
            ...
            if (this.errorState && this.errorState.length) {
                ...
                if (this.ignoreFailedPreconditions) {
                ...
                //don't show warnings here - they should be shown in the warning panel
                const warningIndex: number = this.errorState.findIndex(err => err.type == ERROR_TYPE.WARNING);
                if (warningIndex !== -1) {
                    this.errorStatesToDisplay.splice(warningIndex, 1);
                    this.errorState = this.errorStatesToDisplay;
                }
